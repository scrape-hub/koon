    ...
    pub fn new(options: Option<KoonOptions>) -> Result<Self> {
        ...
        let mut profile = if let Some(ref json) = opts.profile_json {
            ...
            match opts.browser {
                ...
                Some(Browser::Firefox135) => Firefox::v135_windows(),
                Some(Browser::Firefox147Windows) => Firefox::v147_windows(),
                Some(Browser::Firefox147Macos) => Firefox::v147_macos(),
                Some(Browser::Firefox147Linux) => Firefox::v147_linux(),
                Some(Browser::Firefox147) | Some(Browser::Firefox) => Firefox::latest(),
